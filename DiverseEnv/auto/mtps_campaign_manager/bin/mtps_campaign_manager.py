...
def launch_experiment(exp_mode, cmd, exp_dir, num_run):
    print("\tRunning cmd %s" % cmd)
    print("\tExperiment dir %s" % exp_dir)
    p = ProcessRunner(exp_mode, cmd, ".", "", exp_dir, 350, num_run)
    p.start()
    p.join()
    ...
    for combo in combinations:
        ...
        if not os.path.exists(result_dir + os.sep + "done.txt"):
            ...
            cmd = cmd_path + os.sep + "mtps_driver.py --num_run=" + str(
                ...
                350) + " --script_path " + campaign.script_path + " --scenario_name " + campaign.scenario + " --route_name " + campaign.route_name + " --base_path2 " + campaign_params["basePath2"] + " --mitigation_mode " + mitigation_mode
            launch_experiment("param_sweep", cmd, result_dir, num_run)
