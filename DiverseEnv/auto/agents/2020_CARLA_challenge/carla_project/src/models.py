...
class SegmentationModelQValue(torch.nn.Module):
    def __init__(self, input_channels=3, n_steps=4, batch_norm=True, hack=False, temperature=1.0, output_dim=1):
        super().__init__()
        self.temperature = temperature
        self.hack = hack
        self.norm = torch.nn.BatchNorm2d(input_channels) if batch_norm else lambda x: x
        ...
        self.network.backbone.conv1 = torch.nn.Conv2d(
                ...
                kernel_size=old.kernel_size, stride=old.stride,
                padding=old.padding, bias=old.bias)
        self.linear_reduce = torch.nn.Linear(144*256, 16)
        self.linear_out = torch.nn.Linear(16, output_dim)
