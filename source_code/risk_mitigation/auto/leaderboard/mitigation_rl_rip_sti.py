...
class CarlaRIP(object):
    def __init__(self):
        self.config_dict = {
            ...
            'inference_model_path': "/home/sheng/projects/ReachML/reachml/checkpoints/final_lead_ghost_cutin_cvctr_3s_train_2022-05-15-16-17-17.pth",
            ...
            'action_space': 2,
            'base_path': '/home/sheng/projects/auto',
            'ram_shared_path': '/media/sheng/data4/projects/DiverseEnv/auto/ram_shared/dqn_sti_online'
        }
    ...
    def inference_single_episode_rl_mitigation(self, episode, skip_first_seconds,
                                               skip_mitigation_seconds, rl_mitigation=True,
                                               const_threshold=0.3):
        # set policy net mode
        # self.policy_net.eval()
        ...
        carla_server = self.start_sim()
        time.sleep(3)
        self.gym_environment = oatomobile.envs.CARLAEnv(town="Town05", carla_server=carla_server, server_port=2000, fps=40)
        time.sleep(1)
        gym_observation = self.gym_environment.reset()
        self.done = False
        self.rip_models = [oatomobile.baselines.torch.ImitativeModel() for _ in range(4)]
        self.rip_ckpts = ["/media/sheng/data4/projects/Oatomobile/model-400-1.pt",
                          "/media/sheng/data4/projects/Oatomobile/model-400-2.pt",
                          "/media/sheng/data4/projects/Oatomobile/model-400-3.pt",
                          "/media/sheng/data4/projects/Oatomobile/model-400-4.pt"]
        for model, ckpt in zip(self.rip_models, self.rip_ckpts):
            model.load_state_dict(torch.load(ckpt))
        self.rip_agent = oatomobile.baselines.torch.RIPAgent(
            environment=self.gym_environment,
            models=self.rip_models,
            algorithm="WCM",
        )

        # setup inference
        self.carla_rl_env = CarlaRLEnv(config_dict=self.config_dict, gym_environment=self.gym_environment)
        self.carla_rl_env.prepare_step(self.gym_environment)
        ...
        while not done:
            ...
            # also get the gym observation from the gym interface
            # gym_observation = self.gym_environment.observe(None)
            if self.carla_rl_env.scene_env.manager.prev_observation is not None:
                gym_observation = self.carla_rl_env.scene_env.manager.prev_observation
            ...
            if first_act:
                ...
                # points, (target_cam, _) = self.policy_net(img, target)
                # print("Update first action.")
                vehicle_control = self.rip_agent.act(gym_observation)
            ...
            print("Action:", vehicle_control)
            action = vehicle_control

            # insert mitigation action
            if mitigation_action == 1:
                action.brake = 1.0
                action.throttle = 0.0

            # done = self.carla_rl_env.step_act(timestamp, action)
            done = self.carla_rl_env.step_act_vehicle_control(timestamp, action, self.gym_environment)

            # lidar1 = gym_observation['lidar'][:,:,0]
            # lidar2 = gym_observation['lidar'][:,:,1]
            # plt.imshow(lidar1)
            # plt.show()
            # plt.imshow(lidar2)
            # plt.show()
            ...
            # gym visualization
            self.gym_environment.render(mode="human")
    ...
    @staticmethod
    def epsilon_by_frame(frames, epsilon_final=0.01, epsilon_start=0.99, epsilon_decay=10):
    ...
    def start_sim(self):
        self.stop_sim()
        carla_server = subprocess.Popen(self.sim_start)
        time.sleep(3)
        return carla_server
...
def main():
    ...
    #                                                     save_frequency=50)
    # dqn_lbc.training_loop_standalone_process(total_episodes=110,
    #                                          batch_size=32,
    ...
    #                                          load_checkpoint=-1)
    # dqn_lbc.inference_loop_standalone_process(total_episodes=5, mitigation="rip", mitigation_configs={
    #     "skip_mitigation_seconds": 5,
    #     "skip_first_seconds": 4
    # })
    dqn_lbc.inference_loop_standalone_process(total_episodes=5, mitigation="rip", mitigation_configs={
