...
class CarlaLBCDQN(object):
    def __init__(self):
        self.scene_list = [
            '/home/sheng/projects/DiverseEnv/auto/agents/2020_CARLA_challenge/leaderboard/data/fi_ghost_cutin.json',
            '/home/sheng/projects/DiverseEnv/auto/agents/2020_CARLA_challenge/leaderboard/data/fi_lead_slowdown.json'
        ]
        ...
        self.config_dict = {
            ...
            'record': '',
            'timeout': 250,
            'sim_data_save': '/home/sheng/projects/DiverseEnv/auto/sim_data_collection',
            'routes': '/home/sheng/projects/DiverseEnv/auto/agents/2020_CARLA_challenge/leaderboard/data/routes_fi/route_highway.xml',
            'scenarios': '/home/sheng/projects/DiverseEnv/auto/agents/2020_CARLA_challenge/leaderboard/data/fi_ghost_cutin.json',
            'repetitions': 1,
            'agent': 'image_agent_rl.py',
            'agent_config': '/home/sheng/projects/DiverseEnv/auto/agents/2020_CARLA_challenge/epoch24.ckpt',
            'track': 'SENSORS',
            'resume': False,
            ...
            'risk_pickle_stored': "/home/sheng/projects/DiverseEnv/auto/sim_risk_analyzed/risk_lookup_3_risk_analysis.pkl",
            'traj_pickle_stored': "/home/sheng/projects/DiverseEnv/carladataset/carla-sim/safety-critical-simdata/route_highway.xml_fi_lead_cutin.json.pkl",
            'inference_model_path': "/home/sheng/projects/ReachML/reachml/checkpoints/final_lead_slowdown_ghost_cutin_cvctr_3s_train_2022-07-20-22-56-33.pth",
            'inference_config_path': "/home/sheng/projects/ReachML/reachml/data/train_test_sim_lite/route_highway.xml_fi_ghost_cutin.json.pkl/model_unc/data_realtime_trail0_3s/realtime_trail0_3s_data_config.json",
            'qvalue_net_rl': 1e-5,
            'weight_decay': 0.9999,
            'reward_decay': 0.99,
            'tau': 0.01,
            "rl_checkpoint_savedir": "/home/sheng/projects/DiverseEnv/auto/rl_checkpoint",
            'replay_buffer_size': 10000,
            'action_space': 2,
            'base_path': '/home/sheng/projects/DiverseEnv/auto',
            'ram_shared_path': '/media/sheng/data9/home/sheng/DiverseAV/auto/ram_shared/dqn_tcc_c'
        }
    ...
    def inference_single_episode_rl_mitigation(self, episode, skip_first_seconds, skip_mitigation_seconds, rl_mitigation=True):
        # set policy net mode
        self.policy_net.eval()
            ...
            if rewards[0] is not None and rewards[1] is not None:
                ...
                if timestamp.elapsed_seconds > skip_mitigation_seconds:
                    ...
                    if rl_mitigation:
                        ...
                        # self.latency_list.append(time.time() - start_inf)
        ...
        print("mean inference time:", np.mean(self.latency_list), "sem", sem(np.array(self.latency_list)))
    ...
    def get_action(self, states, epsilon, frame_counter, risk, inference=False):
        with torch.no_grad():
            ...
            if random.random() >= epsilon or inference:
                start_inf = time.time()
                imgs = states[0]
                targets = states[1]
                imgs = imgs[None, :]
    ...
    @staticmethod
    def epsilon_by_frame(frames, epsilon_final=0.01, epsilon_start=0.9, epsilon_decay=2000):
    ...
    def save_replay_buffer(self, episode):
        if episode > 0:
            ...
            old_replay_buffer_pickle = os.path.join(self.ram_shared_path, "replay_buffer.{}.pkl".format(episode - 1))
    ...
    def training_loop_standalone_process(
            ...
            skip_mitigation_seconds, save_frequency, resume=0, target_net_update_freq=100, clear_replay_buffer=False, load_checkpoint=-1, interleave=False):
        self.all_rewards = []
        self.avg_rewards = []
        self.total_frame_counter = 0
        ...
        for episode in range(resume, total_episodes):
            ...
            p = Process(
                target=self.training_single_episode_reinforcement_mitigation, args=(
                    ...
                    save_frequency, mitigation_penalty, clear_replay_buffer, interleave)
            )
            p.start()
            p.join()
            self.stop_sim()
    ...
    def interleave_scene(self):
        self.scene_index += 1
        if self.scene_index >= len(self.scene_list):
            self.scene_index = 0
        scene_json = self.scene_list[self.scene_index]
        print("Current scene:", scene_json)
        self.config_dict["scenarios"] = scene_json
    ...
    def training_single_episode_reinforcement_mitigation(
            ...
            skip_mitigation_seconds, save_frequency, mitigation_penalty, clear_replay_buffer=False, interleave=False):
        ...
        if clear_replay_buffer and (not self.replay_buffer_cleared or self.first_run):
            ...
            for i in range(int(len(self.replay_buffer) * 1.00)):
        ...
        if interleave:
            if save_dict is not None:
                self.scene_index = save_dict["scene_index"]
            self.interleave_scene()
        self.carla_rl_env = CarlaRLEnv(config_dict=self.config_dict)
        self.carla_rl_env.prepare_step()
        prev_state_list = list()
        prev_states = None
    ...
    def load_checkpoint(self, episode, prefix="state_dicts"):
        if episode >= 0:
            print("Load network state dicts.")
            state_dicts_pickle = os.path.join(self.ram_shared_path, "{}.{}.pkl".format(prefix, episode))
            f = open(state_dicts_pickle, "rb")
            state_dicts = pickle.load(f)
            f.close()
...
def main():
    ...
    #                                                     save_frequency=50)
    # dqn_lbc.training_loop_standalone_process(total_episodes=50,
    #                                          batch_size=32,
    #                                          start_training_replaybuffer_size=256,
    ...
    #                                          load_checkpoint=-1,
    #                                          interleave=False)
    dqn_lbc.inference_loop_standalone_process(total_episodes=10, mitigation="smart", mitigation_configs={
        "skip_mitigation_seconds": 5,
        "skip_first_seconds": 4
    })
