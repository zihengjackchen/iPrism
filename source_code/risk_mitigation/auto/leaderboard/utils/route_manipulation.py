...
def interpolate_trajectory_wp(world, waypoints_trajectory, hop_resolution):
    """
    Given some raw keypoints interpolate a full dense trajectory to be used by the user.
    returns the full interpolated route both in GPS coordinates and also in its original form.
    ...
    for i in range(len(waypoints_trajectory) - 1):  # Goes until the one before the last.
        ...
        for wp_tuple in interpolated_trace:
            route.append((wp_tuple[0].transform, wp_tuple[1]))
            wps.append(wp_tuple[0])

    lat_ref, lon_ref = _get_latlon_ref(world)

    return location_route_to_gps(route, lat_ref, lon_ref), route, wps
