...
class CarlaLBCDDPG(object):
    ...
    def inference_step_loop_threshold_mitigation(self, risk_threshold=0.9):
        self.policy_net.eval()
        for i in range(1):
            if PLOT:
                plt.figure(figsize=(5, 2.5))
    ...
    @staticmethod
    def inject_action_noise(points, mode="brake",
                            scales=(0.0, 0.0), scale_factors=(1.0, 1.1, 1.2, 1.3)):
        raise NotImplementedError
        return points
        assert mode in ["brake", "three-dim"]
        modified_action = points.cpu().clone().detach()
        if mode == "brake" or mode == "three-dim":
    ...
    def training_step_loop_reinforcement_mitigation(self, episodes, batch_size, skip_first_seconds, save_frequency):
        all_rewards = []
        avg_rewards = []
        for episode in range(episodes):
            ...
            while not done:
                ...
                if len(self.replay_buffer) > batch_size:
                    # parameters_dict = dict()
                    # for name, param in self.qvalue_net.net.named_parameters():
                    #     parameters_dict[name] = param.cpu().clone().detach()
            ...
            if episode % save_frequency == 0:
                self.save_checkpoint(episode, all_rewards[-1])
        print("Doen training {} episodes.".format(episodes))
        self.save_checkpoint("final", all_rewards[-1])
...
def main():
    ...
    # rl_lbc.training_step_loop_reinforcement_mitigation(512, 8, 3.5, 50)
