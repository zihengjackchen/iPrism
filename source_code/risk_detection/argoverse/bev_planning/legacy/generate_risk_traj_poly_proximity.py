...
class GenerateRisk:
    ...
    def _riskAnalyserOneTimestamp(self, timestamp, lookForwardDist, lookForwardTime,
                                  minDist=5, distanceToStaOb=0.25, distanceToDynOb=1.75):
        ...
        timeBasedObstacles = self._getTimeBasedObstacles(listOfTime, staticObstaclesPoly,
                                                         None, None)
        # prepare arguments for proximity execution
        skipGoalIndex = []
        for o in range(len(setOfGoals)):
            if o in skipGoalIndex:
                print("Skipping goal:", o)
                continue
            emptyArgument = dict()
            ...
            success, plannedTraj = self._fot_inference_wrapper(emptyArgument)
            if success:
                emptySuccess += 1
                successGoals.append(referenceWaypoints)
                skipGoalIndex.append(r)
            if len(plannedTraj.x) and len(plannedTraj.y) and len(plannedTraj.x) == len(plannedTraj.y):
                for r in range(len(setOfGoals)):
                    if r in skipGoalIndex:
                        continue
                    goal = setOfGoals[r][-1]
                    for x, y in zip(plannedTraj.x, plannedTraj.y):
                        if np.linalg.norm(np.array(goal) - np.array([x, y])) <= fotHyperparameters["GOAL_PROXIMITY"]:
                            emptySuccess += 1
                            successGoals.append(setOfGoals[r])
                            skipGoalIndex.append(r)
                            break
        if not emptySuccess:
        ...
        for o in range(len(setOfGoalsAllObstacles)):
            ...
            success, plannedTraj = self._fot_inference_wrapper(fullArgument)
            if success:
                fullSuccess += 1
                successGoals.append(referenceWaypoints)
                skipGoalIndex.append(r)
            if len(plannedTraj.x) and len(plannedTraj.y) and len(plannedTraj.x) == len(plannedTraj.y):
                for r in range(len(setOfGoalsAllObstacles)):
                    if r in skipGoalIndex:
                        continue
                    goal = setOfGoalsAllObstacles[r][-1]
                    for x, y in zip(plannedTraj.x, plannedTraj.y):
                        if np.linalg.norm(np.array(goal) - np.array([x, y])) <= fotHyperparameters["GOAL_PROXIMITY"]:
                            fullSuccess += 1
                            successGoals.append(setOfGoalsAllObstacles[r])
                            skipGoalIndex.append(r)
                            break
        ...
        for k in range(len(uuidCurrentTime)):
            ...
            for o in range(len(setOfGoalsLessOneObstacles)):
                ...
                success, plannedTraj = self._fot_inference_wrapper(lessOneArgument)
                if success:
                    lessOneSuccess += 1
                    successGoals.append(referenceWaypoints)
                    skipGoalIndex.append(r)
                if len(plannedTraj.x) and len(plannedTraj.y) and len(plannedTraj.x) == len(plannedTraj.y):
                    for r in range(len(setOfGoalsLessOneObstacles)):
                        if r in skipGoalIndex:
                            continue
                        goal = setOfGoalsLessOneObstacles[r][-1]
                        for x, y in zip(plannedTraj.x, plannedTraj.y):
                            if np.linalg.norm(np.array(goal) - np.array([x, y])) <= fotHyperparameters["GOAL_PROXIMITY"]:
                                lessOneSuccess += 1
                                successGoals.append(setOfGoalsLessOneObstacles[r])
                                skipGoalIndex.append(r)
                                break
            if emptySuccess:
