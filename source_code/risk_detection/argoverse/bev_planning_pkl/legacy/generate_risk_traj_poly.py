...
class GenerateRisk:
    ...
    def riskAnalyser(self, saveFileName=None, lookForwardDist=100, lookForwardTime=3):
    ...
    def _riskAnalyserOneTimestamp(self, timestamp, lookForwardDist, lookForwardTime,
                                  minDist=5, distanceToStaOb=0.15, distanceToDynOb=1.25):
        ...
        # TODO instead of getting rid of an obstacle per time step, we need to get rid of an
        ...
        for tdx in range(len(dynamicObstacles)):
            ...
            if self.posUnc == "None":
                ...
                for obstacle in allDynamicObstacles:
                    ...
                    # xmax, xmin = np.max(xs), np.min(xs)
                    # ymax, ymin = np.max(ys), np.min(ys)
                    # currentDynamicObstaclesXY.append([xmin, ymin, xmax, ymax])
                ...
                for obstacle in allDynamicObstacles:
                    ...
                    if self._obstacleInLane(self.rasterizedOptimizedLanesAroundTraj, transBboxPolygon):
                        bboxPolygon = transBboxPolygon.buffer(distanceToDynOb, cap_style=3)
                        bboxPolygon = translate(bboxPolygon, xoff=inverseEgoTranslate[0], yoff=inverseEgoTranslate[1])
                        bboxPolygon = rotate(bboxPolygon, angle=inverseEgoAngle, origin=(0, 0))
                        # xs, ys = bboxPolygon.exterior.xy
                        # xmax, xmin = np.max(xs), np.min(xs)
            ...
            # TODO instead of gaussian noise over corners, we need to append random small angle rotation
        ...
        fotHyperparameters = {
            ...
            "MAX_ACCEL": 4.0,
            "MAX_CURVATURE": 0.15,
            "MAX_ROAD_WIDTH": 15,  # over estimation
            "D_ROAD_W": 1,
            "DT": averageTimeTick,
            "MAX_T": lookForwardTime + averageTimeTick + 0.00001,
            "MIN_T": lookForwardTime + averageTimeTick,
            "TARGET_SPEED": 50 / 3.6,  # max speed
            "D_T_S": 2.0 / 3.6,
            "N_S_SAMPLE": 5,
            "GOAL_THRESHOLD": 2.0,
            "K_J": 0.01,
        ...
        for o in range(len(setOfGoalsAllObstacles)):
            ...
            # self.routingVisualizer(timeBasedObstacles,
            #                        origin,
            #                        [setOfGoalsAllObstacles[o]],
            #                        self.routeVisDir,
            #                        "{}_full_{}".format(timestamp, o),
            #                        [],
            #                        [resultX, resultY],
            #                        refTraj)
            if success:
        ...
        for k in range(len(uuidCurrentTime)):
            ...
            timeBasedObstaclesLessOne = self._getTimeBasedObstacles(listOfTime, staticObstaclesPoly,
                                                                    dynamicObstaclesLessOnePoly, dynamicObstaclesUUID,
                                                                    [uuid])
            for o in range(len(setOfGoalsLessOneObstacles)):
                planner = (self.callPlanner())()
                referenceWaypoints = setOfGoalsLessOneObstacles[o]
                plannedTraj, refTraj, success = planner.inference(fotInitialState, referenceWaypoints,
                                                                  timeBasedObstaclesLessOne, fotHyperparameters)
                resultX = plannedTraj.x
                resultY = plannedTraj.y
                # self.routingVisualizer(timeBasedObstacles,
                #                        origin,
                #                        [setOfGoalsLessOneObstacles[o]],
                #                        self.routeVisDir,
                #                        "{}_lessone_{}_{}".format(timestamp, k, o),
                #                        [uuid],
                #                        [resultX, resultY],
                #                        refTraj)
                if success:
    ...
    def routingVisualizer(self, obstacles, origin, goals, savedir, savename, removed=[], routes=None, refRoutes=None,
                          visibleRange=80):
