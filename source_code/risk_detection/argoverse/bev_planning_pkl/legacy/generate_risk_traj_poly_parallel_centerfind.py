...
class GenerateRisk:
    ...
    def _checkChildLane(self, egoPosition, POI):
        egoLaneObject, _, _ = self.bevGraph.argoverseMap.get_nearest_centerline(egoPosition, self.bevGraph.cityName, False)
        poiLaneObject, _, _ = self.bevGraph.argoverseMap.get_nearest_centerline(POI, self.bevGraph.cityName, False)
        startLaneID = egoLaneObject.id
        endLaneID = poiLaneObject.id
        if startLaneID == endLaneID:
            return [startLaneID]
        currentLaneID = startLaneID
        count = 10000  # here we prevent going in a loop
        passLandIDs = []
        while currentLaneID and count:
            if currentLaneID == endLaneID:
                return passLandIDs
            passLandIDs.append(currentLaneID)
            currentLaneID = self.bevGraph.argoverseMap.get_lane_segment_successor_ids(currentLaneID, self.bevGraph.cityName)
            count -= 1
        return []

    def _convertChildPOItoRefPath(self, passLaneIDs, egoPosition, POI):
        refTraj = []
        _, _, refPoints = self.bevGraph.argoverseMap.get_nearest_centerline(passLaneIDs[0], self.bevGraph.cityName, False)
        refPoints = interp_arc(50, refPoints[:, 0], refPoints[:, 1])
        minIdx = -1
        minDist = sys.float_info.max
        for idx, point in refPoints:
            if np.linalg.norm(point - egoPosition) <= minDist:
                minDist = np.linalg.norm(point - egoPosition)
                minIdx = idx
        if minIdx > -1:
            refTraj.extend(refPoints[minIdx:])
        else:
            refTraj.append(egoPosition)
        for i in range(1, len(passLaneIDs) - 1, 1):
            currentLaneID = passLaneIDs[i]
            _, _, refPoints = self.bevGraph.argoverseMap.get_nearest_centerline(currentLaneID, self.bevGraph.cityName, False)
            refPoints = interp_arc(50, refPoints[:, 0], refPoints[:, 1])
            refTraj.extend(refPoints)
        _, _, refPoints = self.bevGraph.argoverseMap.get_nearest_centerline(passLaneIDs[-1], self.bevGraph.cityName, False)
        refPoints = interp_arc(50, refPoints[:, 0], refPoints[:, 1])
        minIdx = -1
        minDist = sys.float_info.max
        for idx, point in refPoints:
            if np.linalg.norm(point - POI) <= minDist:
                minDist = np.linalg.norm(point - egoPosition)
                minIdx = idx
        if minIdx > -1:
            refTraj.extend(refPoints[0:minIdx + 1])
        else:
            refTraj.append(POI)
        return refTraj
            ...
            if len(lanePOI):
                ...
                for idx, POI in enumerate(lanePOI):
                    if minDist < POI[0] <= lookForwardDist:
                        passLaneIDs = self._checkChildLane(poseCitytoEgo["translation"][0:2], lanePOIOrin[idx][0:2])
                        if not len(passLaneIDs):  # POI not part of successor lanes
                            POI = self._convertPOItoRefPath(poseCitytoEgo["translation"][0:2], lanePOIOrin[idx][0:2])
                        else:
                            POI = self._convertChildPOItoRefPath(passLaneIDs, poseCitytoEgo["translation"][0:2], lanePOIOrin[idx][0:2])
                        POI = np.array(POI)
