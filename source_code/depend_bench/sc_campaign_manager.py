...
import sys
import glob
from sc_driver import ProcessRunner
...
def launch_epxeriment(exp_mode, cmd, exp_dir, num_run):
    ...
    p = ProcessRunner(exp_mode, cmd, ".", "", exp_dir, 300, num_run)
    p.start()
    p.join()
    print("Experiment {} ended.".format(num_run))


def launch_scenario(base_path, scenario, campaign_params, script_path, output_dir):
    ...
    for combo in combinations:
        ...
        if not os.path.exists(result_dir + os.sep + "done.txt"):
            ...
            cmd_path = campaign.base_path + os.sep + "sc_campaign_manager" + os.sep + "bin/"
            cmd = cmd_path + os.sep + "sc_driver.py --num_run=" + str(
                num_run) + " --agent_mode " + campaign.agent_mode + " --base_path " + campaign.base_path + " --result_dir " + result_dir + " --timeout " + str(
                180) + " --script_path " + campaign.script_path + " --scenario_name " + campaign.scenario + " --route_name " + campaign.route_name + " --base_path2 " + campaign_params["basePath2"]
...
def main():
    ...
    for campaign in campaign_parameters["campaigns"]:
        ...
        launch_scenario(base_path, scenario, campaign, script_path, params.output_dir)
