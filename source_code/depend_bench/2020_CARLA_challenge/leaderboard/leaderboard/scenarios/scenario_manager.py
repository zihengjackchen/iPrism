    ...
    def __init__(
        ...
        if risk_evaluation_mode == "dyn":
            ...
            self.risk_evaluation = GenerateRiskCarlaSimRuntime(
                "fot*", "simrun",
                "/<PATH_TO_FILE>/DiverseEnv/auto/routing_visualizations",
                "/<PATH_TO_FILE>/DiverseEnv/auto/risk_visualizations_rl",
                "/<PATH_TO_FILE>/DiverseEnv/auto/analysis_risk_save",
                1,
                visualize=False
            )
            ...
            self.risk_evaluation2 = GenerateRiskCarlaSimRuntime(
                "fot*", "simrun",
                "/<PATH_TO_FILE>/DiverseEnv/auto/routing_visualizations",
                "/<PATH_TO_FILE>/DiverseEnv/auto/risk_visualizations",
                "/<PATH_TO_FILE>/DiverseEnv/auto/analysis_risk_save",
                1
            )
            ...
            self.risk_evaluation = GenerateRiskCarlaSimRuntimeNN(
                ...
                "/<PATH_TO_FILE>/DiverseEnv/auto/risk_visualizations",
                "/<PATH_TO_FILE>/DiverseEnv/auto/analysis_risk_save",
                "/<PATH_TO_FILE>/DiverseEnv/auto/sim_generated_data",
                "/<PATH_TO_FILE>/DiverseEnv/auto/sim_generated_data_viz",
    ...
    def _calculate_reward(
        ...
        if (len(self.data_collector.timestamp) - 1) % 4 != 0 or (len(self.data_collector.timestamp) - 1) == 0:
            ...
            if mode == "risk":
                ...
                scale_factors["path_complete"] = 0.15
                scale_factors["raw_risk"] = 0.7
                # scale_factors["risk"] = 0.3
                scale_factors["risk"] = 1

                reward_risk = (1 - risk * scale_factors["raw_risk"]) * scale_factors["risk"] * (np.linalg.norm(ego_vector) / self.expected_cover)
                reward_complete = path_complete * scale_factors["path_complete"]
                reward_comfort = comfort_kpi * scale_factors["comfort_kpi"]
                reward = reward_risk + reward_complete + reward_comfort

                print(f"Debug: Current reward: {reward} (raw risk: {(1 - risk * scale_factors['raw_risk']) * scale_factors['risk']} with total_risk: {reward_risk}, completeness: {reward_complete}, comfort: {reward_comfort}")
