...
class SensorInterface(object):
    def __init__(self):
        ...
        self._queue_timeout = 10000 #SJ 10
        ...
        self._duplicate_data = False

    def set_dual_agent(self, mode, duplicate):
        self._dual_agent = mode
        self._new_data_buffers2 = Queue()
        if duplicate:
            print("Alwasys put into the first queue and duplicated for both agent to consume.")
            self._dual_agent = False
    ...
    def duplicate_queue_data(self):
        if self._duplicate_data:
            while self._new_data_buffers.qsize() < 6:
                pass
            for item in self._new_data_buffers.queue:
                self._new_data_buffers2.put(item)
            for i in range(self._new_data_buffers2.qsize()):
                item = self._new_data_buffers2.get(True, self._queue_timeout)
                self._new_data_buffers.put(item)
