...
class OppositeDriftIn(BasicScenario):
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True,
        ...
        self._first_vehicle_location = 125
        ...
        super(OppositeDriftIn, self).__init__("FollowVehicle",
                                                   ego_vehicles,
                                                   config,
                                                   world,
                                                   debug_mode,
    ...
    def _initialize_actors(self, config):
        ...
        self.first_vehicle_waypoint = first_vehicle_waypoint
        first_vehicle_waypoint.transform.rotation.yaw += 180
        self._other_actor_transform = carla.Transform(
            carla.Location(first_vehicle_waypoint.transform.location.x + 3,
                           first_vehicle_waypoint.transform.location.y,
                           first_vehicle_waypoint.transform.location.z + 0.1),
                           first_vehicle_waypoint.transform.rotation)
        self._other_actor_transform.rotation.yaw += 180
    ...
    def _create_behavior(self):
        ...
        starting_point = carla.Location(x=self._other_actor_transform.location.x, y=self._other_actor_transform.location.y, z=0)
        plan_list_straight = list()
        plan_list_straight.append(starting_point)
        for y_offset in range(0, 45, 1):
            plan_list_straight.append(carla.Location(x=self._other_actor_transform.location.x,
                                                     y=self._other_actor_transform.location.y - y_offset, z=0))
        
        last_point = plan_list_straight[-1]
        plan_list_drifting = list()
        for i in range(15):
            last_point = carla.Location(x=last_point.x-0.13, y=last_point.y-0.7, z=last_point.z)
            plan_list_drifting.append(last_point)
        
        last_point = plan_list_drifting[-1]
        for y_offset in range(0, 5, 1):
            last_point = carla.Location(x=193.09947204589844, y=last_point.y - y_offset, z=0)
            plan_list_drifting.append(last_point)

        plan_list = plan_list_straight + plan_list_drifting
        ...
        driving_to_next_intersection_first.add_child(InTriggerDistanceToVehicle(self.other_actors[0],
                                                                          ...
                                                                          distance=300,
                                                                          name="Distance"))
        driving_to_next_intersection_first.add_child(WaypointFollower(self.other_actors[0],
                                                                      plan=plan_list,
                                                                      target_speed=self._first_vehicle_speed))
        driving_to_next_intersection_first.add_child(StopVehicle(self.other_actors[0], self._other_actor_max_brake))
        ...
        endcondition = py_trees.composites.Parallel("Waiting for end position",
                                                    policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ALL)
        endcondition_part = StandStill(self.ego_vehicles[0], name="StandStill", duration=1)
